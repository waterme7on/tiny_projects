...
class MyCapture:
    ...
    def __init__(self):
        ...
        self.capture_text_box.place(
            x=20, y=70, anchor='nw', width=170, height=330)

        self.translate_text_box = tkinter.Text(window)  # 创建text容器用于存放翻译后的文字
        self.translate_text_box.place(
            x=210, y=70, anchor='nw', width=170, height=330)

        self.capture_btn = tkinter.Button(
            text='截图', command=self.capture_cmd)  # 创建一个按钮
        # 在创建的窗口的西北角x=20,y=10处放置按钮
        self.capture_btn.place(x=80, y=10, anchor='nw', width=60, height=20)

        self.capture_btn = tkinter.Button(
            text='翻译', command=self.translate_cmd)  # 创建一个按钮
    ...
    def create_canvas(self):
        ...
        self.top = tkinter.Toplevel(
            window, width=self.screenWidth, height=self.screenHeight)
        # 不显示最大化、最小化按钮
        self.top.overrideredirect(True)
        self.canvas = tkinter.Canvas(
            self.top, bg='white', width=self.screenWidth, height=self.screenHeight)
        # 显示全屏截图，在全屏截图上进行区域截图
