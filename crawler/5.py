#!/usr/bin/env python
# coding: utf-8

# In[1]:


# 提供的excel所在路径
file_name='data.xlsx'


# In[ ]:


file_name=input('输入xlsx文件所在路径')
sheet_name=input('输入爬取子表名')


# In[29]:

'''
crawl_columns=input('输入包含的列（链接、选项1、选项2、...、备注对应的列名）')
crawl_columns=crawl_columns.split("\t")
crawl_columns=[c for c in crawl_columns if c != '']
'''
# print(crawl_columns)


# In[ ]:





# In[ ]:





# In[2]:


# 代理的名字和用户密码
# ADSL 账户和密码
# dial_name = "VPN"
# dial_username = "051511259526"
# dial_passwd = "010203"

dial_name = input('输入VPN名')
dial_username = input('输入VPN账号')
dial_passwd = input('输入VPN密码')


# ## 设置代理

# In[3]:

# ## 取得货品状态后对原excel表格进行更新

# In[24]:


# 对表中颜色进行分析
import openpyxl
from openpyxl.styles import PatternFill, colors

work_book = openpyxl.load_workbook(file_name)
#work_sheet = work_book.get_sheet_by_name('产品目录')
work_sheet = work_book[sheet_name]
print(work_sheet.max_row,work_sheet.max_column)





import os
import time
import signal

def connect_vpn():
    cmd_str = "rasdial %s %s %s" % (dial_name, dial_username, dial_passwd)
    os.system(cmd_str)
    time.sleep(3)
    
def disconnect_vpn():
    cmd_str = "rasdial %s /disconnect" % dial_name
    os.system(cmd_str)
    time.sleep(1)


# In[4]:



#connect_vpn()


# ## 1. 在表中获取各个链接

# In[5]:


import pandas as pd
import numpy as np
import json
import re
from selenium.webdriver.common.proxy import Proxy, ProxyType


# In[6]:


data = pd.read_excel(open(file_name, 'rb'), sheet_name=sheet_name)
data = data.fillna('')


# In[7]:





# In[8]:


sub_data = data[['网站链接', '网站选项1', '网站选项2', '网站选项3','网站选项4','网站选项5','网站选项6','网站选项7','网站选项8','网站选项9','网站选项10','网站选项11','网站选项12','网站选项13','网站选项14','网站选项15','网站选项16','备注']]
# sub_data = data[crawl_columns]
# sub_data = sub_data.drop_duplicates() # 去重

start=input('开始行数:')
end=input('结束行数:')
sub_data = sub_data[int(start)-2: int(end)-1]
# In[9]:


sub_data.head()


# In[10]:


# for idx, row in sub_data.iterrows():
#     print(idx)
#     color = row[1]
#     print(color)
#     option = [i for i in row[2:].to_list() if i]
#     print(option)
#     notes = row['备注']
#     print(notes)
#     break
# # 0
# # 1 混彩色猫眼
# # ['4mm', '6mm', '8mm', '10mm', '12mm', '14mm', '16mm', '无']
# # 无


#    

# ## 2. 获得链接后用爬虫爬取

# In[11]:


from selenium import webdriver
from selenium.webdriver.chrome.options import Options

status_dict = {}
status_urls = []

proxy_id = 0

chrome_options = Options()
#chrome_options.add_argument('--headless')
#chrome_options.add_argument('--disable-gpu')
chrome_driverPath = input('输入ChromeDriver所在路径')
# browser = webdriver.Chrome('C:\Program Files (x86)\Google\Chrome\Application\chromedriver.exe', options=chrome_options)
browser = webdriver.Chrome(chrome_driverPath, options=chrome_options)
browser.delete_all_cookies()
browser.maximize_window()


# In[25]:

def save_to_file():

    red_rgb='ffff0000'
    blue_rgb='ff00b0f0'
    yellow_rgb='ffffff00'

    fill_red = PatternFill('solid', fgColor=red_rgb)
    fill_blue = PatternFill('solid', fgColor=blue_rgb)
    fill_yellow = PatternFill('solid', fgColor=yellow_rgb)
    fill_white = PatternFill('none')
    # isFirstTime = True

    pre_color = {}

    # print(status_urls)
    # 遍历提供的表格并进行修改
    for i in range(1, work_sheet.max_row):
        # 当前行对应的颜色
        row_color = ""
        row_url = ""
        for j in range(1, work_sheet.max_column):
            _cell = (work_sheet.cell(i, j))
            if not _cell.value:
                continue
            # 如果当前颜色在我们爬取结果中，则记录当前行的颜色
            if _cell.value in status_urls:
                row_url = _cell.value
            if row_url == "":
                continue
            if (_cell.value, row_url) in status_dict.keys(): 
                # 颜色不存在或失效则填充为红色
                row_color = _cell.value
                if row_color in abandoned_color:
                    for j in range(1, work_sheet.max_column):
                        work_sheet.cell(i,j).fill = fill_red
                    break
            if row_color == "":
                continue
            # 根据检测之前的颜色和已有状态来变色
            try:
                cell_rolor = str(_cell.fill.fgColor.rgb).lower()
                if _cell.value not in status_dict[(row_color, row_url)].keys():
                    continue
                # 上一次为蓝色的情况
                if cell_rolor.find(blue_rgb)!=-1:
                    # 有货标黄，无货标红
                    if status_dict[(row_color, row_url)][_cell.value]:
                        _cell.fill = fill_yellow
                    else:
                        _cell.fill = fill_red
                # 上一次为黄色的情况
                elif cell_rolor.find(yellow_rgb)!=-1:
                    # 有货标白，无货标蓝
                    if status_dict[(row_color, row_url)][_cell.value]:
                        _cell.fill = fill_white
                    else:
                        _cell.fill = fill_blue
                # 上一次为红色的情况
                elif cell_rolor.find(red_rgb)!=-1:
                    if status_dict[(row_color, row_url)][_cell.value]:
                    # 有货标黄，无货标红
                        _cell.fill = fill_yellow
                    else:
                        _cell.fill = fill_red
                # 上一次为无色的情况
                else:
                    if status_dict[(row_color, row_url)][_cell.value]:
                        # 之前有货，现在有货，依然为无色
                        pass
                    else:
                        # 之前有货，现在无货为蓝色
                        _cell.fill = fill_blue
    #                     # 如果是第一次，无货则直接置为红色
    #                     # 不是第一次无货则置为蓝色
    #                     if isFirstTime:
    #                         _cell.fill = fill_red
    #                     else:
    #                         _cell.fill = fill_blue
            except Exception as e:
                pass

    work_book.save(file_name)
    print("结果已保存到excel")


def handler(signum, frame):
    save_to_file()
    print("爬取终止")
    exit(0)

signal.signal(signal.SIGINT, handler)

# In[21]:

abandoned_color= []

# 打开VPN
print('爬取开始')
#connect_vpn()
# disconnect_vpn()
for idx, row in sub_data.iterrows():
    # 获取该行对应的颜色
    color = row[1]
    # 获取对应颜色下各个型号
    options = [i for i in row[2:-2].to_list() if i]
    notes = row['备注']
    
    # 浏览器爬取
    # 进入网站
    url = row['网站链接']
    if url == '' or not url:
        continue
    if browser.current_url != url:
        browser.get(url)
    # 等待加载的时间
    browser.implicitly_wait(10)

    if url not in status_urls:
        status_urls.append(url)
    status_dict[(color, url)] = {}
    for op in options:
        status_dict[(color, url)][op] = 0

   
    while True:
        try:
            # 查看是否遭到检测
            # 遭到的换ip
            bar = browser.find_element_by_css_selector('#err')
            print("爬取",color,"遭到检测，换ip")
            disconnect_vpn()
            connect_vpn()
            browser.get(url)
            # 等待加载的时间
            browser.implicitly_wait(10)
        except:
            # 未遭到检测则停止
            break
    
    
    print('爬取url: {} 颜色：{}'.format (url, row[1]))
    # 链接失效
    try:
        unabled_url = browser.find_element_by_css_selector("#site_content > div.grid-main > div > div.mod.mod-offerDetailContext1.app-offerDetailContext1.app-type-default.mod-ui-not-show-title > div > div.m-content > div.widget-custom.offerdetail_ditto_offline > div > div")
        abandoned_color.append(color)
    except: 
        pass


    # 只有选项一
    if len(options) == 0:
        # 展开详情页
        try:
            contents_blk = browser.find_element_by_css_selector("#mod-detail-bd > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-sku > div.obj-content")
            browser.execute_script("arguments[0].style = 'height: 1500px;';", contents_blk)
            # 等待加载的时间
            time.sleep(2)
        except:
            pass
        
        # 获得各个颜色选项
        _options = browser.find_elements_by_css_selector('#mod-detail-bd > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-sku > div.obj-content > table > tbody > tr')
        for _op in _options:
            if not _op.text:
                continue
            _color = (_op.get_attribute('data-sku-config'))
            _color = json.loads(_color)['skuName']
            _num = (_op.text.strip('\n-\n+').split(' '))[1]
            if color != _color and color.find(_color) == -1:
                continue
            # 不匹配的可以跳过
            
            if int(re.findall(r"\d+\.?\d*",_num)[0]) <= 0:
                # 记录无货信息
                status_dict[(color, url)][color] = 0
            else:
                # 记录有货信息
                status_dict[(color, url)][color] = int(re.findall(r"\d+\.?\d*",_num)[0])
        print(status_dict[(color, url)])
        continue
    
    # 只有选项二
    if color == '':
        # 展开详情页
        try:
            contents_blk = browser.find_element_by_css_selector('#mod-detail-bd > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-sku > div.obj-content')
            browser.execute_script("arguments[0].style = 'height: 800px;';", contents_blk)
            # 等待加载的时间
            time.sleep(2)
        except:
            pass
        # 展开详情页
        try:
            contents_blk = browser.find_element_by_css_selector("#mod-detail-bd > div.detail-v2018-layout-left > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical.dsc-version2018-page-fix-content-mid > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-sku > div.obj-content")
            browser.execute_script("arguments[0].style = 'height: 800px;';", contents_blk)
            # 等待加载的时间
            time.sleep(2)
        except:
            pass
        
        # 得到当前各个型号
        _options = browser.find_elements_by_css_selector('#mod-detail-bd > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-sku > div.obj-content > table > tbody > tr')
        _options += browser.find_elements_by_css_selector('#mod-detail-bd > div.detail-v2018-layout-left > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical.dsc-version2018-page-fix-content-mid > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-sku > div.obj-content > table > tbody > tr')
        # 遍历各个型号
        for _option in _options:
            # 为空则退出
            if not _option.text: 
                continue
            _option = (_option.text.strip('\n-\n+').split(' '))
            print(color, _option)

            # 不在选项中则退出
            # 可能为空块
            # 对size设置过大
            try:
                __op = _option[0]
                __num = _option[len(_option)-1]
            except:
                continue
#             print(color, __op, __num)
            # 如果不在，查看有无间接匹配的
            if __op not in options:
                __oo = ""
                __op_num = int(re.findall(r"\d+\.?\d*",__op)[0])
                for ___op in options:
                    ___op_num = int(re.findall(r"\d+\.?\d*",___op)[0])
                    if ___op_num == __op_num:
                        __oo = ___op
                        break
                    if __op.find(___op) != -1 or ___op.find(__op) != -1:
                        __oo = ___op
                        break
                if not __oo:
                    continue
                __op = __oo

            if int(re.findall(r"\d+\.?\d*",__num)[0]) <= 0:
                # 记录无货信息
                status_dict[(color, url)][__op] = 0
            else:
                # 记录有货信息
                status_dict[(color, url)][__op] = int(re.findall(r"\d+\.?\d*",__num)[0])

        # 新增：根据备注来进行调节
        # 如果备注否，则说明全部没有货，显示有误
        # 如果备注具体型号，则具体型号没有货
        if notes == '否':
            for option in options:
                status_dict[(color, url)][option] = 0
            continue
        elif notes != '':
            empty_options = notes.split(',')
            for option in empty_options:
                option = option.strip(' ')
                status_dict[(color, url)][option] = 0
        print(status_dict[(color, url)])
        continue
    
    
    # 有选项一和选项二
    # 获取页面中各个元素
    # 这个获取的部分可能随时会变
    
    # 有图片的
    alters = browser.find_elements_by_css_selector('#mod-detail-bd > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-leading > div.obj-content > ul > li > div > a > span.vertical-img > span > img')
    alters += browser.find_elements_by_css_selector('#mod-detail-bd > div.detail-v2018-layout-left > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical.dsc-version2018-page-fix-content-mid > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-leading > div.obj-content > ul > li > div > a > span.vertical-img > span > img')
    # 只有文字的
    alters += browser.find_elements_by_css_selector('#mod-detail-bd > div.detail-v2018-layout-left > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical.dsc-version2018-page-fix-content-mid > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-leading > div.obj-content > ul > li > div > a')
    alters += browser.find_elements_by_css_selector('#mod-detail-bd > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-leading > div.obj-content > ul > li > div > a')
    
    tmp_color = color
    tmp_color = tmp_color.replace(" ","")
    for alter in alters:
        _color = ''
        if color == '':
            _color = alter.get_attribute('alt')
        if not _color:
            _color = alter.get_attribute('title')
        if not _color:
            _color = alter.text.strip(" |\n")
        if not _color:
            continue
        _tmp_color = _color
        _tmp_color = _tmp_color.replace(" ","")
        # 如果当前行的颜色匹配，则点击，否则跳过
        # if color != _color and _color.find(color) == -1 and color.find(_color) == -1 and _tmp_color != tmp_color and _tmp_color.find(tmp_color) == -1 and tmp_color.find(_tmp_color) == -1:
        #    continue
        if color != _color:
            continue
        
        print(_color)
        print(options)
        
        try:
            alter.click()
        except:
            continue
        # 等待加载的时间
        
        browser.implicitly_wait(2)
        time.sleep(3)

        # 展开详情页
        try:
            contents_blk = browser.find_element_by_css_selector('#mod-detail-bd > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-sku > div.obj-content')
            browser.execute_script("arguments[0].style = 'height: 800px;';", contents_blk)
            # 等待加载的时间
            time.sleep(2)
        except:
            pass
        # 展开详情页
        try:
            contents_blk = browser.find_element_by_css_selector("#mod-detail-bd > div.detail-v2018-layout-left > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical.dsc-version2018-page-fix-content-mid > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-sku > div.obj-content")
            browser.execute_script("arguments[0].style = 'height: 800px;';", contents_blk)
            # 等待加载的时间
            time.sleep(2)
        except:
            pass
    
    
        # 得到当前颜色下各个型号
#         _options = browser.find_elements_by_css_selector('#mod-detail-bd > div.detail-v2018-layout-left > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical.dsc-version2018-page-fix-content-mid > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-sku > div.obj-content > table > tbody > tr')
        _options = browser.find_elements_by_css_selector('#mod-detail-bd > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-sku > div.obj-content > table > tbody > tr')
        if not _options:
            _options = browser.find_elements_by_css_selector('#mod-detail-bd > div.detail-v2018-layout-left > div.region-custom.region-detail-property.region-takla.ui-sortable.region-vertical.dsc-version2018-page-fix-content-mid > div.widget-custom.offerdetail_ditto_purchasing > div > div > div > div.obj-sku > div.obj-content > table > tbody > tr')
        # 遍历各个型号
        for _option in _options:
            # 为空则退出
            if not _option.text: 
                continue
            _option = (_option.text.strip('\n-\n+').split(' '))
            print(color, _option)

            # 不在选项中则退出
            # 可能为空块
            # 对size设置过大
            try:
                __op = _option[0]
                __num = _option[len(_option)-1]
            except:
                continue
#             print(color, __op, __num)
            # 如果不在，查看有无间接匹配的
            if __op not in options:
                __oo = ""
                __op_num = int(re.findall(r"\d+\.?\d*",__op)[0])
                for ___op in options:
                    ___op_num = int(re.findall(r"\d+\.?\d*",___op)[0])
                    if ___op_num == __op_num:
                        __oo = ___op
                        print(__oo, ___op)
                        break
                if not __oo:
                    continue
                __op = __oo

            if int(re.findall(r"\d+\.?\d*",__num)[0]) <= 0:
                # 记录无货信息
                status_dict[(color, url)][__op] = 0
            else:
                # 记录有货信息
                status_dict[(color, url)][__op] = int(re.findall(r"\d+\.?\d*",__num)[0])

        # 新增：根据备注来进行调节
        # 如果备注否，则说明全部没有货，显示有误
        # 如果备注具体型号，则具体型号没有货
        if notes == '否':
            for option in options:
                status_dict[(color, url)][option] = 0
            continue
        elif notes != '':
            empty_options = notes.split(',')
            for option in empty_options:
                option = option.strip(' ')
                status_dict[(color, url)][option] = 0
        print(status_dict[(color, url)])
        break
disconnect_vpn()


# In[16]:


browser.close()


# In[22]:


# print(status_dict)


# In[23]:


# import json
# with open('data.json', 'w', encoding='utf8') as f:
#     json.dump(status_dict, f)



# In[ ]:


save_to_file()


