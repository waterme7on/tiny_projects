...
class Face_Register:
    ...
    def draw_note(self, img_rd):
        ...
        if self.press_n_flag == 0:
            cv2.putText(img_rd, "N: create personal folder", (20, 350),self.font, 0.8, (255, 255, 255), 1, cv2.LINE_AA)
        else:
            cv2.putText(img_rd, "S: save face images", (20, 350),self.font, 0.8, (255, 255, 255), 1, cv2.LINE_AA)
        if self.press_s_flag:
            cv2.putText(img_rd, "Q: complete register", (20, 400), self.font, 0.8, (255, 255, 255), 1, cv2.LINE_AA)
    ...
    def process(self, stream):
        ...
        while stream.isOpened():
            ...
            if kk == ord('n'):
                ...
                current_face_dir = self.path_photos_from_camera + \
                            ...
                            if self.press_n_flag:
                                ...
                                for ii in range(height*2):
                                    for jj in range(width*2):
                                        img_blank[ii][jj] = img_rd[d.top(
                                        )-hh + ii][d.left()-ww + jj]
                                cv2.imwrite(
                                    current_face_dir + "/img_face_" + str(self.ss_cnt) + ".jpg", img_blank)
                                self.press_s_flag = 1
                                print("写入本地 / Save into：", str(current_face_dir) +
                                      "/img_face_" + str(self.ss_cnt) + ".jpg")
                            else:
                                print(
                                    "请先按 'N' 来建文件夹, 按 'S' / Please press 'N' and press 'S'")
